...
export function FlashcardVocabBlinderGame({ flashcards, onClose, onNext, onSelectNewGame }: FlashcardVocabBlinderGameProps) {
  ...
  if (isGameComplete) {
    ...
    return (
      <div className="fixed inset-0 bg-gradient-to-br from-indigo-50 via-purple-50 to-indigo-100 dark:from-indigo-950 dark:via-purple-900 dark:to-indigo-950 overflow-auto flex items-center justify-center p-2 sm:p-4">
        ...
        <Card className="max-w-md w-full shadow-2xl relative z-10 bg-white/90 backdrop-blur-xl border-white/50 rounded-2xl sm:rounded-[2rem]">
          <CardHeader className="text-center pb-2 pt-4 sm:pt-6">
            ...
            <CardTitle className="text-xl sm:text-2xl font-bold text-foreground bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">
          ...
          <CardContent className="space-y-3 sm:space-y-4 p-4 sm:p-6">
            <div className="bg-gradient-to-r from-indigo-500 to-purple-500 rounded-xl sm:rounded-2xl p-3 sm:p-4 text-white text-center shadow-lg transform hover:scale-105 transition-transform duration-300">
              <div className="text-3xl sm:text-4xl font-bold mb-1">{score}</div>
              <div className="text-xs sm:text-sm opacity-90 font-medium">{t('games.totalScore')}</div>
            </div>

            <div className="grid grid-cols-3 gap-2 sm:gap-3">
              <div className="text-center p-2 sm:p-3 bg-green-50 dark:bg-green-900/30 rounded-lg sm:rounded-xl border border-green-100">
                <div className="text-xl sm:text-2xl font-bold text-green-600 dark:text-green-400">
                  {correctCount}
                </div>
                <div className="text-[10px] sm:text-xs text-green-700 dark:text-green-300 mt-1 font-medium">
                  {t('common.correct')}
                </div>
              </div>
              <div className="text-center p-2 sm:p-3 bg-red-50 dark:bg-red-900/30 rounded-lg sm:rounded-xl border border-red-100">
                <div className="text-xl sm:text-2xl font-bold text-red-600 dark:text-red-400">
                  {wrongCount}
                </div>
                <div className="text-[10px] sm:text-xs text-red-700 dark:text-red-300 mt-1 font-medium">
                  {t('common.incorrect')}
                </div>
              </div>
              <div className="text-center p-2 sm:p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg sm:rounded-xl border border-blue-100">
                <div className="text-xl sm:text-2xl font-bold text-blue-600 dark:text-blue-400">
                  {accuracy}%
                </div>
                <div className="text-[10px] sm:text-xs text-blue-700 dark:text-blue-300 mt-1 font-medium">
                  {t('common.accuracy')}
                </div>
              </div>
            </div>
            ...
            {wrongWords.length > 0 ? (
              <div className="bg-red-50 dark:bg-red-900/20 rounded-lg sm:rounded-xl p-3 sm:p-4 border border-red-100">
                <div className="flex items-center gap-2 mb-2 sm:mb-3">
                  ...
                  <span className="font-bold text-sm sm:text-base text-red-700 dark:text-red-300">{t('games.wrongWords')} ({wrongWords.length})</span>
                </div>
