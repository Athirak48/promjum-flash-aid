...
interface ListeningMCQPhaseProps {
    vocabList: VocabWord[];
...
export function ListeningMCQPhase({
    vocabList,
    audioSettings,
    onComplete,
    onClose
}: ListeningMCQPhaseProps) {
        ...
        <div className="relative h-full flex flex-col bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 dark:from-slate-900 dark:via-purple-950 dark:to-indigo-950 overflow-hidden font-sans">
            ...
            <div className="flex-1 flex flex-col items-center justify-start gap-3 px-4 py-2 z-10 w-full max-w-lg mx-auto overflow-hidden">
                ...
                <div className="flex items-center justify-center gap-3 w-full">
                    {/* Rabbit Button - Normal Speed (1x) */}
                    <motion.button
                        onClick={() => handlePlayAudio(1.0)}
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                        className={`w-12 h-12 rounded-full shadow-md flex items-center justify-center text-xl transition-all
                            ${currentSpeed === 1.0
                                ? 'bg-gradient-to-br from-pink-400 to-rose-500 ring-2 ring-pink-200 ring-offset-2'
                    ...
                    <motion.button
                        ...
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                        className="relative w-16 h-16 rounded-full shadow-lg bg-gradient-to-br from-indigo-500 via-purple-500 to-fuchsia-500 border-3 border-white ring-2 ring-indigo-100 flex items-center justify-center"
                    >
                        {isPlaying ? (
                    ...
                    {/* Turtle Button - Slow Speed (0.75x) */}
                    <motion.button
                    ...
                    {question.options.map((option, index) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <span className={`font-medium text-sm flex-1 ${showStatus && isCorrectOption ? 'text-white' : 'text-slate-700'}`}>
                                    {option}
                                </span>
                            </motion.button>
                        );
